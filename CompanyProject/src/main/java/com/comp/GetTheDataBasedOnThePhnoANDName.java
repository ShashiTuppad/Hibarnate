...
public class GetTheDataBasedOnThePhnoANDName {
	public static void main(String[] args) {
		EntityManagerFactory entityManagerFactory=Persistence.createEntityManagerFactory("venkySir");
		EntityManager entityManager=entityManagerFactory.createEntityManager();
		
		 Query query=entityManager.createQuery("select c from Company c where c.phno=?1 and c.cname=?2");
		 query.setParameter(1,9999999999l);
		 query.setParameter(2,"HeroHonda");
		
		
	        List<Company> company = query.getResultList();
	        
	        for(Company company1: company) {
	        	System.out.println("The company Name: "+company1.getCid());
